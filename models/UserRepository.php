...
class userRepository
{
    ...
    {
        $db = Conectar::conexion();
        $result = $db->query("INSERT INTO users(user, password, rol, conectado) VALUES ('$user', md5('$password'), 1, 0)");
    }
    ...
    public static function getUserOnline($sala)
    {
        $online = [];
        $tiempoDesconexion = 60; //Se desconectara en 1 minuto
        $db = Conectar::conexion();
        $result = $db->query("SELECT * FROM users WHERE conectado = 1 and sala=" . $sala);
        while ($datos = $result->fetch_assoc())
        {
            $user = new User($datos);
            if (time() - $user->getTime() < $tiempoDesconexion)
                $online[] = new User($datos); //Si han pasado menos de 60 segundos se añade al array de usuarios
        }
        return $online;
    }
